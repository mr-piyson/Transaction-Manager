...
function RoleIcon({
  ...
  return (
    <div
      className="size-6 rounded flex items-center justify-center text-white text-xs font-bold shrink-0 shadow-sm transition-transform hover:scale-105"
...
export default function UsersSettingsPage() {
  ...
  const { data: editingRolePerms, isLoading: permsLoading } =
  ...
  if (
    rolePermsDialogOpen &&
    editingRolePerms &&
    editingRoleId !== prevRoleRef.current
  ) {
  ...
  const validateForm = useCallback((): boolean => {
    ...
    if (!form.name.trim())
      errors.name = t("errors.requiredField", { field: t("users.name") });
    if (!form.email.trim())
      errors.email = t("errors.requiredField", { field: t("users.email") });
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email))
      errors.email = t("errors.invalidEmail");
    if (!form.roleId)
      errors.roleId = t("errors.requiredField", { field: t("users.role") });
    if (!isEditing && form.password && form.password.length < 6) {
      errors.password = t("errors.minLength", {
        field: t("users.password"),
        min: 6,
      });
    }
  ...
  const handleSetPassword = useCallback(() => {
    if (!newPassword || newPassword.length < 6) {
      setPasswordError(
        t("errors.minLength", { field: t("users.password"), min: 6 }),
      );
      return;
    }
  ...
  const selectedRoleName = form.roleId
    ? (roleMap.get(form.roleId)?.name ?? "")
    : "";
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/10 hover:bg-muted/10">
                    ...
                    <TableHead className="w-[80px] text-right font-semibold pr-4">
                      {t("common.actions")}
                    </TableHead>
                  </TableRow>
                </TableHeader>
                              ...
                              <HoverCardContent className="w-80 p-4 border bg-popover text-popover-foreground rounded-xl shadow-xl z-50">
                                <div className="space-y-3">
                                  <div className="flex items-center gap-2.5">
                                    <div
                                      className="size-8 rounded-lg flex items-center justify-center text-white text-base font-bold"
                                      style={{ backgroundColor: roleColor }}
                                    >
                                      {(() => {
                                        const iconMap: Record<string, string> =
                        ...
                        <TableCell className="text-right pr-4">
                          <DropdownMenu>
                            ...
                            >
                              ...
                              <DropdownMenuItem
                                onClick={() => openSetPasswordDialog(user.id)}
                              >
                              ...
                              <DropdownMenuItem
                                ...
                                disabled={
                                  ...
                                  sendPasswordResetMutation.variables?.id ===
                                    user.id
                                }
                              >
                                {sendPasswordResetMutation.isPending &&
        ...
        <DialogContent className="sm:max-w-xl max-h-[80vh] flex flex-col">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {editingRoleId &&
                t("users.permissionsForRole", {
                  role: roleMap.get(editingRoleId)?.name ?? "",
                })}
            </DialogTitle>
            <DialogDescription>
            ...
            <div key={module} className="space-y-1">
              ...
              <div className="space-y-0.5">
                {(perms as any[]).map((perm: any) => {
                  const permId =
                    allPermissions?.find((p: any) => p.code === perm.code)
                      ?.id ?? "";
                  return (
                    <Label
                      key={perm.code}
