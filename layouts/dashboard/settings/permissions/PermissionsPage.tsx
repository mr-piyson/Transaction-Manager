...
function RoleIcon({
  ...
  return (
    <div
      className={cn(
        "flex items-center justify-center text-white font-bold shrink-0 shadow-sm transition-transform hover:scale-105",
        sizeClasses[size],
      )}
...
export default function PermissionsPage() {
  ...
  const { data: allPermissions, isLoading: permsLoading } =
    trpc.users.permissions.listAll.useQuery();
  ...
  const { data: editingRolePerms, isLoading: permsForRoleLoading } =
    trpc.users.rolePermissions.list.useQuery(
  ...
  if (
    ...
    editingRolePerms &&
    editingRole &&
    editingRole.id !== prevRoleRef.current
  ) {
  ...
  const toggleSelectAllModule = useCallback(
    (perms: any[]) => {
      const permIds = perms
        .map((p) => allPermissions?.find((ap) => ap.code === p.code)?.id)
  ...
  const handleSaveRolePerms = useCallback(() => {
    ...
    rolePermsUpdateMutation.mutate({
      roleId: editingRole.id,
    ...
    <div className="flex flex-col h-full space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            className="gap-2 shrink-0 shadow-md hover:shadow-lg transition-all duration-200"
            asChild
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {rows.map((role) => {
            ...
            return (
              <Card
                key={role.id}
                className="hover:shadow-md transition-all duration-200 flex flex-col justify-between border bg-card"
              >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <h3 className="text-sm font-semibold truncate max-w-[130px]">
                            {role.name}
                          </h3>
                          {role.isSystem ? (
                            <Badge
                              variant="secondary"
                              className="text-[9px] px-1 py-0 bg-muted text-muted-foreground border-0 font-bold uppercase tracking-wider"
                            >
                              System
                            </Badge>
                          ) : (
              ...
              <div className="grid grid-cols-8 gap-2">
                {AVAILABLE_ICONS.map((icon) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setNewRole((prev) => ({ ...prev, icon: icon.name }))
                    }
                    className={cn(
              ...
              <div className="flex items-center gap-3 flex-wrap">
                {CURATED_COLORS.map((color) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setNewRole((prev) => ({ ...prev, color: color.value }))
                    }
                    className={cn(
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setEditRoleForm((prev) => ({ ...prev, name: e.target.value }))
                }
                placeholder={t("users.roleNamePlaceholder")}
              />
            </div>
              ...
              <div className="grid grid-cols-8 gap-2">
                {AVAILABLE_ICONS.map((icon) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setEditRoleForm((prev) => ({ ...prev, icon: icon.name }))
                    }
                    className={cn(
                      "size-8 rounded-lg flex items-center justify-center text-sm font-semibold border hover:bg-muted/80 transition-colors shadow-sm",
                      editRoleForm.icon === icon.name
                        ? "border-primary bg-primary/10 ring-2 ring-primary/20 scale-105"
                        : "border-input",
                    )}
                    title={icon.label}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!editRoleForm.name.trim() || updateRole.isPending}
            >
        ...
        <DialogContent className="sm:max-w-xl max-h-[85vh] flex flex-col">
          <DialogHeader className="pb-2">
            <DialogTitle className="flex items-center gap-2">
              ...
              {editingRole &&
                t("users.permissionsForRole", { role: editingRole.name })}
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pr-1 py-2">
            {permsForRoleLoading ? (
              ...
              Object.entries(filteredPermissionsByModule).map(
                ([module, perms]) => {
                  const permIds = (perms as any[])
                    .map(
                      (p) =>
                        allPermissions?.find((ap) => ap.code === p.code)?.id,
                    )
                    .filter(Boolean) as string[];
                  const allSelected = permIds.every((id) =>
                    selectedPermIds.includes(id),
                  );
                  const countSelected = permIds.filter((id) =>
                    selectedPermIds.includes(id),
                  ).length;
                  ...
                  return (
                    <div
                      key={module}
                      className="space-y-1.5 border rounded-xl p-3 bg-muted/5"
                    >
...
function RoleCardPermissionsPreview({ roleId, t }: { roleId: string; t: any }) {
  const { data: perms, isLoading } = trpc.users.rolePermissions.list.useQuery(
    { roleId },
    { enabled: !!roleId },
  );
        ...
        >
          {expanded ? (
            ...
            <>
              ...
              Show {moduleKeys.length - 2} more groups
            </>
          )}
        </button>
      )}
