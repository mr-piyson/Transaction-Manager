					...
					<div className="space-y-4 pr-4">
						...
						<div>
							...
							<div className="space-y-2">
								...
								<DetailRow
									...
									value={format(item.averageCost)}
								/>
								<DetailRow
									label={t("common.tax")}
									value={item.taxRateName ?? "—"}
								/>
						...
						<div>
							...
							<div className="space-y-2">
								...
								/>
								<DetailRow
									label={t("items.minStock")}
									value={`${item.minStock}`}
								/>
								<DetailRow
									label={t("items.reorderAt")}
									...
									{item.stockByWarehouse.map((sw) => (
										...
										>
											...
											<span
												className={cn(
													"font-medium",
													sw.quantity <= 0 && "text-destructive",
												)}
											>
...
export default function ItemReportPage() {
	...
	const kpis = useMemo(() => {
		...
		const avgPrice =
			items.length > 0
				? items.reduce((s, i) => s + i.salesPrice, 0) / items.length
				: 0;
	...
	const warehouseChartData = useMemo(() => {
		...
		for (const item of items) {
			for (const sw of item.stockByWarehouse) {
				map.set(
					sw.warehouseName,
					(map.get(sw.warehouseName) ?? 0) + sw.quantity,
				);
			}
		}
		return Array.from(map.entries())
			...
			},
			{
				...
				cellRenderer: (params: { value: string }) => {
					...
					return (
						<Badge
							variant="outline"
							className={cn(" px-1.5 py-0 font-medium", c.className)}
						>
							{c.label}
						</Badge>
					);
