...
export default function PayrollDetailPage() {
			...
			<div className="flex items-center justify-center h-[60vh]">
				<Empty>
					<EmptyHeader>
						...
						<EmptyDescription>
							The payroll run you're looking for doesn't exist or has been
							deleted.
						</EmptyDescription>
					</EmptyHeader>
				</Empty>
	...
	const totalDeductions = items.reduce(
		(s: number, i: any) => s + Number(i.deductions ?? 0),
		0,
	);
	const totalNet = items.reduce(
		(s: number, i: any) => s + Number(i.netPay ?? 0),
		0,
	);
					...
					<div className="flex items-center gap-4">
						...
						<div>
							<div className="flex items-center gap-2">
								...
								<Badge
									variant="outline"
									className={STATUS_COLORS[payroll.status] ?? ""}
								>
									{payroll.status}
								</Badge>
							</div>
										...
										<TableRow key={item.id}>
											...
											<TableCell className="font-mono text-sm">
												{item.baseSalary != null
													? Number(item.baseSalary).toFixed(3)
													: "—"}
											</TableCell>
											<TableCell className="font-mono text-sm">
												{item.allowances != null
													? Number(item.allowances).toFixed(3)
													: "—"}
											</TableCell>
											<TableCell className="font-mono text-sm">
												{item.deductions != null
													? Number(item.deductions).toFixed(3)
													: "—"}
											</TableCell>
											<TableCell className="font-mono text-sm font-medium">
												{item.netPay != null
													? Number(item.netPay).toFixed(3)
													: "—"}
											</TableCell>
						...
						<div className="grid grid-cols-2 gap-4">
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										setEditingItem({ ...editingItem, bonus: e.target.value })
									}
								/>
							</div>
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										setEditingItem({ ...editingItem, overtime: e.target.value })
									}
								/>
							</div>
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										setEditingItem({ ...editingItem, tax: e.target.value })
									}
								/>
							</div>
