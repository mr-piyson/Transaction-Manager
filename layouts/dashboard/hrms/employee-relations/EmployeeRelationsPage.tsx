...
export default function GrievancesPage() {
	...
	const { data, isLoading, refetch } =
		trpc.hr.employeeRelations.grievances.list.useQuery({
			page,
			limit,
	...
	const deleteMutation =
		trpc.hr.employeeRelations.grievances.delete.useMutation({
			onSuccess: () => {
				refetch();
	...
	const updateAssigneeMutation =
		trpc.hr.employeeRelations.grievances.assign.useMutation({
			onSuccess: () => {
				refetch();
	...
	const updateStatusMutation =
		trpc.hr.employeeRelations.grievances.updateStatus.useMutation({
			onSuccess: () => {
				refetch();
	...
	return (
		<div className="space-y-6">
									...
									{grievances.map((g: any) => (
										<TableRow key={g.id}>
											...
											<TableCell>
												<Badge
													variant="outline"
													className={statusBadge[g.status] ?? ""}
												>
													{g.status}
												</Badge>
											</TableCell>
											<TableCell className="text-sm">
												{g.assignee?.user?.name ?? "—"}
											</TableCell>
											<TableCell className="text-sm">
											...
											<TableCell>
												<DropdownMenu>
													...
													<DropdownMenuContent align="end">
														...
														>
															<MessageSquare className="size-4 mr-2" /> Change
															Status
														</DropdownMenuItem>
														<DropdownMenuItem
			...
			<Dialog
				open={!!viewGrievance}
				onOpenChange={() => setViewGrievance(null)}
			>
				<DialogContent>
					<DialogHeader>
					...
					{viewGrievance && (
						<div className="space-y-3 text-sm">
							<div>
								<strong>Employee:</strong> {viewGrievance.employee?.user?.name}
							</div>
							<div>
								<strong>Subject:</strong> {viewGrievance.subject}
							</div>
							<div>
								<strong>Description:</strong> {viewGrievance.description ?? "—"}
							</div>
							<div>
								<strong>Status:</strong> {viewGrievance.status}
							</div>
							<div>
								<strong>Assignee:</strong>{" "}
								{viewGrievance.assignee?.user?.name ?? "Unassigned"}
							</div>
							<div>
								<strong>Resolution:</strong> {viewGrievance.resolution ?? "—"}
							</div>
							<div>
								<strong>Created:</strong>{" "}
								{viewGrievance.createdAt
									? formatDateTime(viewGrievance.createdAt)
									: "—"}
							</div>
						</div>
					)}
					<DialogFooter>
						<Button variant="outline" onClick={() => setViewGrievance(null)}>
							Close
						</Button>
					</DialogFooter>
				</DialogContent>
			...
			<Dialog open={assignDialog} onOpenChange={setAssignDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Assign this grievance to an employee for investigation.
						</DialogDescription>
					</DialogHeader>
			...
			<Dialog open={statusDialog} onOpenChange={setStatusDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Change the grievance status and add resolution details.
						</DialogDescription>
					</DialogHeader>
