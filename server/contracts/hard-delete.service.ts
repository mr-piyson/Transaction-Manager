...
export async function getHardDeleteInfo(
  ...
  const [auditLogs, tags, notifications, attachments] = await Promise.all([
    tx.auditLog.count({ where: { entityType: 'Contract', entityId: contractId } }),
    tx.tagging.count({ where: { entityType: 'Contract', entityId: contractId } }),
    tx.notification.count({ where: { entityType: 'Contract', entityId: contractId } }),
    tx.attachment.count({ where: { entityType: 'Contract', entityId: contractId } }),
  ]);
