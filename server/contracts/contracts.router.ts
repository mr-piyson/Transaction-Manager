...
export const contractsRouter = router({
	list: orgProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const where: Record<string, unknown> = {
				...
				...(expiringSoon
					? { endDate: { lte: thirtyDaysFromNow }, status: "ACTIVE" }
					: {}),
				...(search
					? {
							OR: [
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (existing.status === "ACTIVE") {
				throw new UnprocessableError(
					"Active contracts must be terminated before deletion.",
				);
			}
	...
	activate: orgProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (new Date(contract.endDate) <= new Date()) {
				throw new UnprocessableError(
					"Cannot activate a contract whose end date is in the past.",
				);
			}
	...
	renew: orgProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const duration =
				oldEnd.getTime() - new Date(contract.startDate).getTime();
