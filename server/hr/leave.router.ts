...
export const leaveRouter = router({
	...
	types: {
		...
		update: orgProcedure
			.input(
				z.object({ id: z.string() }).merge(leaveTypeCreateSchema.partial()),
			)
			.mutation(async ({ ctx, input }) => {
				const { id, ...data } = input;
				const orgId = ctx.user.organizationId;
				...
				if (data.code) {
					...
					if (dup)
						throw new ConflictError(
							"Another leave type with this code already exists.",
						);
				}
		...
		delete: orgProcedure
			...
			.mutation(async ({ ctx, input }) => {
				...
				const existing = await ctx.db.leaveType.findFirst({
					...
					include: {
						_count: { select: { leaveRequests: true, leaveBalances: true } },
					},
				});
				...
				if (
					...
					throw new UnprocessableError(
						"Cannot delete a leave type that has requests or balances.",
					);
				}
	...
	requests: {
		...
		create: orgProcedure
			...
			.mutation(async ({ ctx, input }) => {
				...
				if (overlapping) {
					throw new ConflictError(
						"Employee already has a pending or approved leave request for this period.",
					);
				}
	...
	holidays: {
		list: orgProcedure
			.input(hrListSchema.extend({ dateRange: dateRangeFilterSchema }))
			.query(async ({ ctx, input }) => {
