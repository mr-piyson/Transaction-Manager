...
export const trainingRouter = router({
	list: orgProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const { search, sortBy, sortOrder, status, isMandatory, ...pagination } =
				input;
			...
			.mutation(async ({ ctx, input }) => {
				...
				if (existing.status !== "PLANNED") {
					throw new UnprocessableError(
						"Cannot unenroll from an in-progress or completed training.",
					);
				}
