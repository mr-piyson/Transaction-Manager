...
export const payrollRouter = router({
	list: orgProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const { search, sortBy, sortOrder, status, dateRange, ...pagination } =
				input;
				...
				include: {
					...
					items: {
						include: {
							...
							payslip: {
								select: { id: true, fileUrl: true, generatedAt: true },
							},
						},
					},
				},
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (overlapping) {
				throw new ConflictError(
					`Overlapping payroll run already exists: "${overlapping.name}".`,
				);
			}
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (run.status !== "DRAFT") {
				throw new UnprocessableError(
					`Cannot process a payroll run in "${run.status}" status.`,
				);
			}
				...
				await writeAuditLog(
					{
						...
						diff: {
							status: { before: run.status, after: "PROCESSING→DRAFT" },
							itemsCreated: { before: 0, after: items.length },
						},
						organizationId: orgId,
						userId: ctx.user.id,
						ipAddress: ctx.ipAddress,
	...
	complete: orgProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (run.status !== "DRAFT") {
				throw new UnprocessableError(
					`Cannot complete a payroll run in "${run.status}" status.`,
				);
			}
			if (run._count.items === 0) {
				throw new UnprocessableError(
					"Cannot complete a payroll run with no items.",
				);
			}
	...
	cancel: orgProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!["DRAFT", "PROCESSING"].includes(run.status)) {
				throw new UnprocessableError(
					`Cannot cancel a payroll run in "${run.status}" status.`,
				);
			}
		...
		.mutation(async ({ ctx, input }) => {
			...
			await ctx.db.$transaction(async (tx) => {
				await tx.payslip.deleteMany({
					where: { payrollItem: { payrollRunId: input.id } },
				});
		...
		update: orgProcedure
			...
			.mutation(async ({ ctx, input }) => {
				...
				if (item.payrollRun.status !== "DRAFT") {
					throw new UnprocessableError(
						"Can only edit items in DRAFT payroll runs.",
					);
				}
