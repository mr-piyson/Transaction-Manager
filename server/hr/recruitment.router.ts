		...
		create: orgProcedure
			...
			.mutation(async ({ ctx, input }) => {
				...
				return ctx.db.jobPosting.create({
					data: { ...input, publishedAt: new Date(), organizationId: orgId },
				});
			}),
			...
			.mutation(async ({ ctx, input }) => {
				...
				if (input.status === "HIRED" && existing.status !== "OFFERED") {
					throw new UnprocessableError(
						"Cannot hire a candidate who has not been offered.",
					);
				}
		...
		offer: {
			...
			create: orgProcedure
				...
				.mutation(async ({ ctx, input }) => {
					...
					if (existingOffer)
						throw new ConflictError(
							"An offer already exists for this candidate.",
						);
			...
			respond: orgProcedure
				...
				.mutation(async ({ ctx, input }) => {
					...
					if (offer.status !== "PENDING") {
						throw new UnprocessableError(
							`Offer is already in "${offer.status}" status.`,
						);
					}
					...
					return ctx.db.$transaction(async (tx) => {
						...
						await tx.candidate.update({
							...
							data: {
								status: input.status === "ACCEPTED" ? "HIRED" : "REJECTED",
							},
						});
			...
			withdraw: orgProcedure
				...
				.mutation(async ({ ctx, input }) => {
					...
					if (!["PENDING", "ACCEPTED"].includes(offer.status)) {
						throw new UnprocessableError(
							"Only PENDING or ACCEPTED offers can be withdrawn.",
						);
					}
