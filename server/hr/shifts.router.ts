...
const shiftUpdateSchema = shiftCreateSchema
	.partial()
	.extend({ id: z.string() });
...
export const shiftsRouter = router({
			...
			if (data.name) {
				...
				if (dup)
					throw new ConflictError(
						"Another shift with this name already exists.",
					);
			}
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (existing._count.employeeShifts > 0) {
				throw new UnprocessableError(
					"Cannot delete a shift that has employee assignments.",
				);
			}
	...
	employeeShifts: {
		...
		upsert: orgProcedure
			.input(employeeShiftUpsertSchema)
			.mutation(async ({ ctx, input }) => {
				const orgId = ctx.user.organizationId;
