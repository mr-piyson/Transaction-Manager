...
export const employeesRouter = router({
	...
	byUser: orgProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			if (!employee)
				throw new NotFoundError("Employee", `user ${input.userId}`);
			return employee;
		}),
		...
		.mutation(async ({ ctx, input }) => {
			...
			const result = await withRetry(() =>
				ctx.db.$transaction(async (tx) => {
					const employeeCode =
						input.employeeCode ?? (await generateEmployeeCode(tx, orgId));
	...
	delete: orgProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (
				...
				throw new UnprocessableError(
					"Cannot delete an employee with leave or payroll records.",
				);
			}
	...
	updateStatus: orgProcedure
		.input(
			z.object({
				id: z.string(),
				newStatus: employeeStatusSchema,
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (existing.status === input.newStatus) {
				throw new UnprocessableError(
					`Employee is already in status "${input.newStatus}".`,
				);
			}
			...
			return ctx.db.$transaction(async (tx) => {
				const updated = await tx.employee.update({
					...
					data: {
						...
						terminationDate:
							input.newStatus === "TERMINATED" || input.newStatus === "RESIGNED"
								? new Date()
								: null,
					},
				});
				...
				await writeAuditLog(
					{
						...
						diff: {
							status: { before: existing.status, after: input.newStatus },
						},
						organizationId: orgId,
						userId: ctx.user.id,
						ipAddress: ctx.ipAddress,
