...
export const performanceRouter = router({
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (existing.status !== "DRAFT") {
				throw new UnprocessableError(
					`Cannot edit a review in "${existing.status}" status.`,
				);
			}
	...
	submit: orgProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (existing.status !== "DRAFT") {
				throw new UnprocessableError(
					`Cannot submit a review in "${existing.status}" status.`,
				);
			}
	...
	acknowledge: orgProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (existing.status !== "SUBMITTED") {
				throw new UnprocessableError(
					`Cannot acknowledge a review in "${existing.status}" status.`,
				);
			}
				...
				await writeAuditLog(
					{
						...
						action: "STATUS_CHANGE",
						diff: {
							status: { before: existing.status, after: "ACKNOWLEDGED" },
						},
						organizationId: orgId,
						userId: ctx.user.id,
						ipAddress: ctx.ipAddress,
