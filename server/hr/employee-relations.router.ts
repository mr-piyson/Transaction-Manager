			...
			.mutation(async ({ ctx, input }) => {
				...
				if (
					...
					throw new UnprocessableError(
						"Resolution is required when resolving or closing a grievance.",
					);
				}
					...
					await writeAuditLog(
						{
							...
							diff: {
								status: { before: grievance.status, after: input.status },
							},
							organizationId: orgId,
							userId: ctx.user.id,
							ipAddress: ctx.ipAddress,
	...
	disciplinaryActions: {
		list: orgProcedure
			...
			.query(async ({ ctx, input }) => {
				...
				const { search, sortBy, sortOrder, employeeId, type, ...pagination } =
					input;
				const { skip, take } = toPrismaPage(pagination);
				const orgId = ctx.user.organizationId;
					...
					if (input.type === "SUSPENSION" || input.type === "TERMINATION") {
						...
						await tx.employee.update({
							...
							data: {
								status: newStatus,
								terminationDate:
									newStatus === "TERMINATED" ? new Date() : undefined,
							},
						});
