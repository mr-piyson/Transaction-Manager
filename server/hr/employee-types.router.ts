...
export const employeeTypesRouter = router({
	...
	create: orgProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			return ctx.db.$transaction(async (tx) => {
				const et = await tx.employeeType.create({
	...
	update: orgProcedure
		.input(z.object({ id: z.string() }).merge(employeeTypeSchema.partial()))
		.mutation(async ({ ctx, input }) => {
			const { id, ...data } = input;
			const orgId = ctx.user.organizationId;
			...
			if (data.name) {
				...
				if (dup)
					throw new ConflictError(
						"Another employee type with this name already exists.",
					);
			}
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (existing._count.employees > 0) {
				throw new UnprocessableError(
					"Cannot delete an employee type that has employees assigned.",
				);
			}
