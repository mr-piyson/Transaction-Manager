...
export const unitsRouter = router({
  ...
  create: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existing)
        throw new ConflictError(`Unit code "${input.code}" already exists.`);
      ...
      if (data.code && data.code !== existing.code) {
        ...
        if (conflict)
          throw new ConflictError(
            `Unit code "${data.code}" is already in use.`,
          );
      }
  ...
  delete: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existing.isDefault)
        throw new ConflictError("Cannot delete the default unit.");
      if (existing.items.length > 0) {
        throw new ConflictError(
          "Cannot delete a unit that is assigned to items.",
        );
      }
