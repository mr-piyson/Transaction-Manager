...
export const purchaseOrdersRouter = router({
  ...
  create: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!warehouse)
        throw new NotFoundError("Warehouse", orderData.warehouseId);
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!["DRAFT", "PENDING_APPROVAL"].includes(existing.status)) {
        throw new UnprocessableError(
          `PO in status "${existing.status}" cannot be edited.`,
        );
      }
  ...
  order: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (po.status !== "APPROVED")
        throw new UnprocessableError(
          `PO must be APPROVED before ordering. Current: ${po.status}`,
        );
      if (po.version !== input.version)
        throw new StaleDataError("PurchaseOrder");
  ...
  pay: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!["RECEIVED", "PARTIAL_RECEIVED", "INVOICED"].includes(po.status)) {
        throw new UnprocessableError(
          `PO in status "${po.status}" cannot be paid.`,
        );
      }
      if (po.version !== input.version)
        throw new StaleDataError("PurchaseOrder");
  ...
  cancel: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (["CANCELLED", "CLOSED", "RECEIVED", "INVOICED"].includes(po.status)) {
        throw new UnprocessableError(
          `PO in status "${po.status}" cannot be cancelled.`,
        );
      }
      if (po.version !== input.version)
        throw new StaleDataError("PurchaseOrder");
