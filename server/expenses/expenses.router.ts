import { z } from 'zod';
import { ConflictError, ForbiddenError, NotFoundError, UnprocessableError } from '@/lib/error';
...
export const expensesRouter = router({
  ...
  byId: orgProcedure.input(z.object({ id: z.string() })).query(async ({ ctx, input }) => {
    ...
    const expense = await ctx.db.expense.findFirst({
      ...
      include: {
        ...
        purchaseOrder: {
          select: { id: true, serial: true, supplier: { select: { id: true, name: true } } },
        },
        createdBy: { select: { id: true, name: true } },
      },
    });
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.categoryId) {
        const category = await ctx.db.expenseCategory.findFirst({
          where: { id: input.categoryId, organizationId: orgId, isActive: true, deletedAt: null },
          select: { id: true },
        });
      ...
      if (input.purchaseOrderId) {
        ...
        if (!po) throw new NotFoundError('PurchaseOrder', input.purchaseOrderId);
      }
  ...
  update: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (data.categoryId) {
        const category = await ctx.db.expenseCategory.findFirst({
          where: { id: data.categoryId, organizationId: orgId, isActive: true, deletedAt: null },
          select: { id: true },
        });
        ...
        if (accountingChanged) {
          ...
          if (existing.journalEntryId) {
            ...
            if (je?.status === 'DRAFT') {
              ...
              await reversePostedEntry(tx, je.id, orgId, ctx.user.id, 'Expense edited');
            }
  ...
  hardDeleteInfo: orgProcedure.input(z.object({ id: z.string() })).query(async ({ ctx, input }) => {
    ...
    const [auditLogs, tags, notifications, attachments] = await Promise.all([
      ctx.db.auditLog.count({ where: { entityType: 'Expense', entityId: input.id } }),
      ctx.db.tagging.count({ where: { entityType: 'Expense', entityId: input.id } }),
      ctx.db.notification.count({ where: { entityType: 'Expense', entityId: input.id } }),
      ctx.db.attachment.count({ where: { entityType: 'Expense', entityId: input.id } }),
    ]);
  ...
  hardDelete: orgProcedure.input(z.object({ id: z.string() })).mutation(async ({ ctx, input }) => {
    ...
    await ctx.db.$transaction(async (tx) => {
      ...
      await tx.auditLog.deleteMany({ where: { entityType: 'Expense', entityId: input.id } });
      await tx.tagging.deleteMany({ where: { entityType: 'Expense', entityId: input.id } });
      await tx.notification.deleteMany({ where: { entityType: 'Expense', entityId: input.id } });
      await tx.attachment.deleteMany({ where: { entityType: 'Expense', entityId: input.id } });
  ...
  categories: router({
    ...
    create: orgProcedure
      .input(z.object({ name: z.string().min(1).max(200), accountId: z.string().optional() }))
      .mutation(async ({ ctx, input }) => {
        assertCan(ctx.ability, 'expense:create', 'Expense');
        ...
        if (input.accountId) {
          const account = await ctx.db.ledgerAccount.findFirst({
            where: { id: input.accountId, organizationId: orgId, isActive: true },
            select: { id: true },
          });
          if (!account) throw new NotFoundError('LedgerAccount', input.accountId);
        }

        return ctx.db.$transaction(async (tx) => {
          const created = await tx.expenseCategory.create({
            data: { name: input.name, accountId: input.accountId, organizationId: orgId },
          });
    ...
    update: orgProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (data.name && data.name !== existing.name) {
          ...
          if (dup) throw new ConflictError(`Expense category "${data.name}" already exists.`);
        }

        if (data.accountId) {
          const account = await ctx.db.ledgerAccount.findFirst({
            where: { id: data.accountId, organizationId: orgId, isActive: true },
            select: { id: true },
          });
          if (!account) throw new NotFoundError('LedgerAccount', data.accountId);
        }
          ...
          await writeAuditLog(
            {
              ...
              entityId: id,
              action: 'UPDATE',
              diff: { name: { before: existing.name, after: data.name ?? existing.name } },
              organizationId: orgId,
              userId: ctx.user.id,
              ipAddress: ctx.ipAddress,
            },
