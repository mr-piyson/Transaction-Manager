...
export const usersRouter = router({
  ...
  delete: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.id === ctx.user.id)
        throw new ForbiddenError("delete", "Cannot delete yourself.");
  ...
  roles: {
    ...
    update: orgProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (role.isSystem)
          throw new ForbiddenError("update", "Cannot modify system roles.");
    ...
    delete: orgProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (role.isSystem)
          throw new ForbiddenError("delete", "Cannot delete system roles.");
  ...
  rolePermissions: {
    ...
    update: orgProcedure
      .input(
        z.object({ roleId: z.string(), permissionIds: z.array(z.string()) }),
      )
      .mutation(async ({ ctx, input }) => {
        assertCan(ctx.ability, "role:manage", "all", {
          organizationId: ctx.user.organizationId,
  ...
  countByRole: orgProcedure.query(async ({ ctx }) => {
    ...
    for (const c of counts) {
      const key =
        roleMap.get(c.roleId!)?.systemKey ??
        roleMap.get(c.roleId!)?.name ??
        c.roleId!;
