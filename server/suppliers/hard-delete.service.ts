...
export async function getHardDeleteInfo(
  ...
  const [supplierItems, purchaseOrders, auditLogs, tags, notifications, attachments] =
    await Promise.all([
      tx.supplierItem.count({ where: { supplierId } }),
      tx.purchaseOrder.count({ where: { supplierId } }),
      tx.auditLog.count({ where: { entityType: 'Supplier', entityId: supplierId } }),
      tx.tagging.count({ where: { entityType: 'Supplier', entityId: supplierId } }),
      tx.notification.count({ where: { entityType: 'Supplier', entityId: supplierId } }),
      tx.attachment.count({ where: { entityType: 'Supplier', entityId: supplierId } }),
    ]);
