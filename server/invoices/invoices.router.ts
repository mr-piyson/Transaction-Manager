		...
		.mutation(async ({ ctx, input }) => {
			...
			if (invoiceData.parentInvoiceId) {
				...
				if (parent.type !== "INVOICE") {
					throw new UnprocessableError(
						"Credit notes can only reference INVOICE type documents.",
					);
				}
				...
				const totalAlreadyCredited = Number(
					existingCreditNotes._sum.total ?? 0,
				);
			...
			if (invoiceData.customerId && !invoiceData.isWalkIn) {
				...
				if (!customer)
					throw new NotFoundError("Customer", invoiceData.customerId);
			}
				...
				try {
					invoice = await ctx.db.$transaction(async (tx) => {
						...
						if (!dueDate && invoiceData.type === "INVOICE") {
							...
							dueDate = new Date(
								(invoiceData.date ?? new Date()).getTime() + terms * 86_400_000,
							);
						}
	...
	send: orgProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (invoice.type === "INVOICE" && !invoice.warehouseId) {
				throw new UnprocessableError(
					"A warehouse must be assigned before sending an invoice.",
				);
			}
			...
			if (
				...
				throw new UnprocessableError(
					"A warehouse must be assigned before sending a credit note.",
				);
			}
				...
				if (invoice.type === "CREDIT_NOTE" && invoice.parentInvoiceId) {
					...
					if (parentInvoice) {
						...
						const amountDue = Math.max(
							0,
							total - amountPaid - totalCreditApplied,
						);
							...
							await writeAuditLog(
								{
									...
									diff: {
										status: { before: parentOldStatus, after: invoiceStatus },
									},
									organizationId: orgId,
									userId: ctx.user.id,
									ipAddress: ctx.ipAddress,
				...
				await writeAuditLog(
					{
						...
						action: "STATUS_CHANGE",
						diff: {
							status: { before: invoice.status, after: "PENDING_APPROVAL" },
						},
						organizationId: orgId,
						userId: ctx.user.id,
						ipAddress: ctx.ipAddress,
			...
			const invoice = await ctx.db.$transaction(async (tx) => {
				...
				const dueDate = new Date(
					(quote.date ?? new Date()).getTime() +
						(org?.paymentTermsDays ?? 30) * 86_400_000,
				);
