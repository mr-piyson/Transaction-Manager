 ...
 */
export async function fetchAllRatesForBase(
  baseCurrency: string,
...
export async function fetchAllCurrencyPairs(): Promise<ExchangeRateData[]> {
  ...
  for (const baseCurrency of allCurrencies) {
    try {
      ...
      console.error(
        `[frankfurter] Failed to fetch rates for base ${baseCurrency}:`,
        error,
      );
    }
  }
 ...
 */
export async function fullSyncCurrenciesAndRates(
  ...
  for (const rate of rateMap.values()) {
    try {
      ...
      console.error(
        `[frankfurter] Failed to upsert rate ${rate.fromCurrency}/${rate.toCurrency}:`,
        error,
      );
    }
  }
  ...
  const uniqueCurrencies = new Set(
    rates.map((r) => r.fromCurrency).concat(rates.map((r) => r.toCurrency)),
  );
