...
export async function hardDeleteItemTree(
  ...
  await tx.auditLog.deleteMany({ where: { entityType: 'Item', entityId: itemId } });
  await tx.tagging.deleteMany({ where: { entityType: 'Item', entityId: itemId } });
  await tx.notification.deleteMany({ where: { entityType: 'Item', entityId: itemId } });
  await tx.attachment.deleteMany({ where: { entityType: 'Item', entityId: itemId } });
