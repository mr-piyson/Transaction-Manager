...
export async function getHardDeleteInfo(
  ...
  const [
    ...
    tx.auditLog.count({ where: { entityType: 'Customer', entityId: customerId } }),
    tx.tagging.count({ where: { entityType: 'Customer', entityId: customerId } }),
    tx.notification.count({ where: { entityType: 'Customer', entityId: customerId } }),
    tx.attachment.count({ where: { entityType: 'Customer', entityId: customerId } }),
  ]);
...
export async function hardDeleteCustomerTree(
  ...
  for (const contract of contracts) {
    await hardDeleteContractTree(tx, organizationId, contract.id, userId, ipAddress);
  }
  ...
  await tx.activity.deleteMany({
    where: {
      OR: [
        ...(contactIdList.length > 0 ? [{ contactId: { in: contactIdList } }] : []),
