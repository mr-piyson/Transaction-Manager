...
async function ensurePermissionsExist() {
  const existingCodes = new Set(
    (await db.permission.findMany({ select: { code: true } })).map(
      (p) => p.code,
    ),
  );
...
async function renameOneToOne() {
  ...
  for (const perm of oldPermissions) {
    ...
    if (existing) {
      console.log(
        `    - ${perm.code}: target ${newCode} already exists, reassigning rolePermissions...`,
      );
...
async function expandOneToMany() {
  ...
  console.log(
    `  ~ Expanding ${oldPermissions.length} composite permissions into granular ones...`,
  );
  ...
  for (const oldPerm of oldPermissions) {
    ...
    if (newPerms.length === 0) {
      console.log(
        `    ! Warning: no target permissions found for ${oldPerm.code}`,
      );
      continue;
    }
...
async function deleteUnmapped() {
  ...
  console.log(
    `  ✓ Deleted ${toDelete.length} permissions and their rolePermissions.`,
  );
}
