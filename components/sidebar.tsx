...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      className={cn(
        // 1. Core Positioning: absolute and full height
        'absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear sm:flex',

        // 2. RTL Fixed Positioning:
        // Use 'inset-inline-end' to stay on the edge of the sidebar regardless of DIR
        'group-data-[side=left]:-inset-inline-end-2 group-data-[side=right]:-inset-inline-start-2',

        // 3. Cursor Logic: Swapping based on state and side
        'in-data-[side=left]:cursor-w-resize in-data-[side=right]:cursor-e-resize',
        '[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize',

        // 4. Interaction: remove translate-x as it fights with logical positioning
        'hover:bg-transparent group-data-[collapsible=offcanvas]:translate-x-0',
        className,
      )}
      {...props}
    />
