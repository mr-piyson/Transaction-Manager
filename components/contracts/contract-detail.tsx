...
export function ContractDetail({ contract }: ContractDetailProps) {
	...
	const isExpiringSoon =
		...
		daysRemaining >= 0 &&
		daysRemaining <= (contract.renewalAlertDays ?? 30);
		...
		<div className="flex-1 overflow-y-auto p-4 space-y-4">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
				...
				<Card>
					<CardHeader className="pb-1.5">
						<CardTitle className="text-xs text-muted-foreground font-medium">
							{t("contracts.contractValue")}
						</CardTitle>
					</CardHeader>
					<CardContent>
						<p className="font-semibold">
							{Number(contract.contractValue ?? 0).toFixed(3)}{" "}
							{contract.currency}
						</p>
					</CardContent>
				</Card>
				...
				<CardContent>
					...
					{isExpiringSoon && (
						<p className="text-xs text-yellow-600 dark:text-yellow-400 mt-1">
							⚠️ {t("contracts.expiringSoon")} ({daysRemaining}{" "}
							{t("contracts.days")})
						</p>
					)}
				</CardContent>
			...
			<div className="text-xs text-muted-foreground flex flex-wrap gap-x-4 gap-y-1 pb-2">
				...
				<span>
					{t("common.updated")}{" "}
					{contract.updatedAt ? formatDateTime(contract.updatedAt) : "—"}
