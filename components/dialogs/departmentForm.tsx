...
export function DepartmentFormDialog({
	...
	const updateMutation = trpc.hr.departments.update.useMutation({
		onSuccess: () => {
			utils.hr.departments.list.invalidate();
			if (department?.id)
				utils.hr.departments.byId.invalidate({ id: department.id });
...
const DepartmentFormContext =
	React.createContext<DepartmentFormContextValue | null>(null);
...
export function DepartmentFormProvider({
	children,
}: {
...
export function useDepartmentForm(): DepartmentFormContextValue {
	...
	if (!ctx)
		throw new Error(
			"useDepartmentForm must be used inside <DepartmentFormProvider>",
		);
	return ctx;
}
