...
export function GrievanceFormDialog({
	...
	const createMutation =
		trpc.hr.employeeRelations.grievances.create.useMutation({
			onSuccess: (res) => {
...
const GrievanceFormContext =
	React.createContext<GrievanceFormContextValue | null>(null);
...
export function GrievanceFormProvider({
	children,
}: {
...
export function useGrievanceForm(): GrievanceFormContextValue {
	...
	if (!ctx)
		throw new Error(
			"useGrievanceForm must be used inside <GrievanceFormProvider>",
		);
	return ctx;
}
