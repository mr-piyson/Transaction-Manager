...
export function DisciplinaryActionFormDialog({
	open,
	onOpenChange,
	onSuccess,
}: DisciplinaryActionFormDialogProps) {
	...
	const createMutation =
		trpc.hr.employeeRelations.disciplinaryActions.create.useMutation({
			onSuccess: (res) => {
...
const DisciplinaryActionFormContext =
	React.createContext<DisciplinaryActionFormContextValue | null>(null);
...
export function DisciplinaryActionFormProvider({
	children,
}: {
...
export function useDisciplinaryActionForm(): DisciplinaryActionFormContextValue {
	...
	if (!ctx)
		throw new Error(
			"useDisciplinaryActionForm must be used inside <DisciplinaryActionFormProvider>",
		);
	return ctx;
}
