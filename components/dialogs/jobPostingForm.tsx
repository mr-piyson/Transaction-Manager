...
export function JobPostingFormDialog({
	...
	const updateMutation = trpc.hr.recruitment.jobPostings.update.useMutation({
		onSuccess: () => {
			utils.hr.recruitment.jobPostings.list.invalidate();
			if (jobPosting?.id)
				utils.hr.recruitment.jobPostings.byId.invalidate({ id: jobPosting.id });
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						...
						<Field orientation="vertical">
							...
							<Select
								...
								onValueChange={(v) =>
									setValue("employmentType", v as any, { shouldValidate: true })
								}
							>
								<SelectTrigger>
									<SelectValue />
...
const JobPostingFormContext =
	React.createContext<JobPostingFormContextValue | null>(null);
...
export function JobPostingFormProvider({
	children,
}: {
...
export function useJobPostingForm(): JobPostingFormContextValue {
	...
	if (!ctx)
		throw new Error(
			"useJobPostingForm must be used inside <JobPostingFormProvider>",
		);
	return ctx;
}
