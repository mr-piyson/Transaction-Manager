...
export function TimePunchFormDialog({
	open,
	onOpenChange,
	onSuccess,
}: TimePunchFormDialogProps) {
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent>
				<DialogHeader>
					...
					<DialogDescription>
						Record a manual time punch for an employee.
					</DialogDescription>
				</DialogHeader>
				...
				<form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
					...
					<Field orientation="vertical">
						...
						<Select
							...
							onValueChange={(v) =>
								setValue("punchState", v, { shouldValidate: true })
							}
						>
							<SelectTrigger>
					...
					<Field orientation="vertical">
						...
						<Input
							id="source"
							{...register("source")}
							placeholder="e.g. Manual, Terminal"
						/>
					</Field>
...
const TimePunchFormContext =
	React.createContext<TimePunchFormContextValue | null>(null);

export function TimePunchFormProvider({
	children,
}: {
	children?: React.ReactNode;
}) {
	const [open, setOpen] = React.useState(false);
	const [onSuccessCb, setOnSuccessCb] = React.useState<
		(() => void) | undefined
	>();
...
export function useTimePunchForm(): TimePunchFormContextValue {
	...
	if (!ctx)
		throw new Error(
			"useTimePunchForm must be used inside <TimePunchFormProvider>",
		);
	return ctx;
}
