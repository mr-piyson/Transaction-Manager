...
export function JobPositionFormDialog({
	...
	const updateMutation = trpc.hr.jobPositions.update.useMutation({
		onSuccess: () => {
			utils.hr.jobPositions.list.invalidate();
			if (jobPosition?.id)
				utils.hr.jobPositions.byId.invalidate({ id: jobPosition.id });
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						...
						<Field orientation="vertical">
							...
							>
								...
								<SelectContent>
									{positions
										.filter((p: any) => p.id !== jobPosition?.id)
										.map((p: any) => (
...
const JobPositionFormContext =
	React.createContext<JobPositionFormContextValue | null>(null);
...
export function JobPositionFormProvider({
	children,
}: {
...
export function useJobPositionForm(): JobPositionFormContextValue {
	...
	if (!ctx)
		throw new Error(
			"useJobPositionForm must be used inside <JobPositionFormProvider>",
		);
	return ctx;
}
