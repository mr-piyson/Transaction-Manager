...
export function LeaveAllocateFormDialog({
	open,
	onOpenChange,
	onSuccess,
}: LeaveAllocateFormDialogProps) {
...
const LeaveAllocateFormContext =
	React.createContext<LeaveAllocateFormContextValue | null>(null);
...
export function LeaveAllocateFormProvider({
	children,
}: {
...
export function useLeaveAllocateForm(): LeaveAllocateFormContextValue {
	...
	if (!ctx)
		throw new Error(
			"useLeaveAllocateForm must be used inside <LeaveAllocateFormProvider>",
		);
	return ctx;
}
