        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Field orientation="vertical">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setValue("fromCurrency", v as CurrencyCode, {
                    shouldValidate: true,
                  })
                }
                disabled={isEdit}
            ...
            <Field orientation="vertical">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setValue("toCurrency", v as CurrencyCode, {
                    shouldValidate: true,
                  })
                }
                disabled={isEdit}
          ...
          {rate?.source && rate.source !== "manual" && (
            <div className="rounded-md bg-muted p-3 text-sm text-muted-foreground">
              This rate was auto-synced from {rate.source}. Editing will convert
              it to a manual rate.
            </div>
          )}
...
function defaults(
  rate?: ExchangeRateDialogProps["rate"],
): Partial<ExchangeRateFormValues> {
  return {
...
const ExchangeRateFormContext =
  React.createContext<ExchangeRateFormContextValue | null>(null);
...
export function ExchangeRateFormProvider({
  ...
  const openEdit = React.useCallback(
    (rate: RateData, options?: OpenOptions) => {
...
export function useExchangeRateForm(): ExchangeRateFormContextValue {
  ...
  if (!ctx)
    throw new Error(
      "useExchangeRateForm must be used inside <ExchangeRateFormProvider>",
    );
  return ctx;
}
