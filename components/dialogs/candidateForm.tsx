...
export function CandidateFormDialog({
	...
	const { data: jobPostingsData } =
		trpc.hr.recruitment.jobPostings.list.useQuery({});
	...
	const updateMutation = trpc.hr.recruitment.candidates.update.useMutation({
		onSuccess: () => {
			utils.hr.recruitment.candidates.list.invalidate();
			if (candidate?.id)
				utils.hr.recruitment.candidates.byId.invalidate({ id: candidate.id });
	...
	const jobPostings = Array.isArray(jobPostingsData)
		? jobPostingsData
		: ((jobPostingsData as any)?.data ?? []);
				...
				<DialogHeader>
					...
					<DialogDescription>
						{isEdit
							...
							: "Fill in the details to create a new candidate."}
					</DialogDescription>
				</DialogHeader>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						...
						<Field orientation="vertical">
							...
							<Select
								...
								onValueChange={(v) =>
									setValue("jobPostingId", v, { shouldValidate: true })
								}
							>
								<SelectTrigger>
...
const CandidateFormContext =
	React.createContext<CandidateFormContextValue | null>(null);
...
export function CandidateFormProvider({
	children,
}: {
...
export function useCandidateForm(): CandidateFormContextValue {
	...
	if (!ctx)
		throw new Error(
			"useCandidateForm must be used inside <CandidateFormProvider>",
		);
	return ctx;
}
