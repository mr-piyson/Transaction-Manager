...
export function EmployeeTypeFormDialog({
	...
	const updateMutation = trpc.hr.employeeTypes.update.useMutation({
		onSuccess: () => {
			utils.hr.employeeTypes.list.invalidate();
			if (employeeType?.id)
				utils.hr.employeeTypes.byId.invalidate({ id: employeeType.id });
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
				<DialogHeader>
					...
					<DialogDescription>
						{isEdit
							? "Update the employee type details below."
...
const EmployeeTypeFormContext =
	React.createContext<EmployeeTypeFormContextValue | null>(null);
...
export function EmployeeTypeFormProvider({
	children,
}: {
...
export function useEmployeeTypeForm(): EmployeeTypeFormContextValue {
	...
	if (!ctx)
		throw new Error(
			"useEmployeeTypeForm must be used inside <EmployeeTypeFormProvider>",
		);
	return ctx;
}
