...
const POLYMORPHIC_ROWS: RelatedRow[] = [
	...
	{ key: "tags", icon: Tags, labelKey: "hardDelete.tags" },
	{
		key: "notifications",
		icon: ShieldAlert,
		labelKey: "hardDelete.notifications",
	},
...
const INVOICE_ROWS: RelatedRow[] = [
	...
	},
	{
		key: "stockMovements",
		icon: Package,
		labelKey: "hardDelete.stockMovements",
	},
	{
		key: "creditNoteAllocations",
		icon: Link2,
		labelKey: "hardDelete.creditNoteAllocations",
	},
	...
	{ key: "conversions", icon: Receipt, labelKey: "hardDelete.conversions" },
	{
		...
		icon: FileClock,
		labelKey: "hardDelete.approvalRequests",
	},
	...POLYMORPHIC_ROWS,
];
...
const ITEM_ROWS: RelatedRow[] = [
	...
	{ key: "priceListLines", icon: Tags, labelKey: "hardDelete.priceListLines" },
	{
		key: "purchaseLines",
		icon: ShoppingCart,
		labelKey: "hardDelete.purchaseLines",
	},
	{
		key: "invoiceLines",
		icon: Receipt,
		labelKey: "hardDelete.invoiceLines",
		unlink: true,
	},
	{
		key: "expenses",
		icon: Wallet,
		labelKey: "hardDelete.expenses",
		unlink: true,
	},
	...POLYMORPHIC_ROWS,
];
...
const CUSTOMER_ROWS: RelatedRow[] = [
	...
	{ key: "crmContacts", icon: Users, labelKey: "hardDelete.contacts" },
	{
		...
		icon: TrendingUp,
		labelKey: "hardDelete.opportunities",
	},
...
const SUPPLIER_ROWS: RelatedRow[] = [
	{ key: "supplierItems", icon: Truck, labelKey: "hardDelete.supplierItems" },
	{
		key: "purchaseOrders",
		icon: ShoppingCart,
		labelKey: "hardDelete.purchaseOrders",
	},
	...POLYMORPHIC_ROWS,
];
...
const KIND_HOOKS: Record<
	HardDeleteKind,
	{ useInfo: InfoHookLike; useDelete: DeleteHookLike }
> = {
	invoice: {
		useInfo: trpc.invoices.hardDeleteInfo.useQuery as unknown as InfoHookLike,
		useDelete: trpc.invoices.hardDelete
...
function RelatedRows({
	...
	const totalRelated = info
		? rows.reduce((sum, row) => sum + Number(info[row.key] ?? 0), 0)
		: 0;
		...
		<Dialog open={open} onOpenChange={(v) => !isPending && onOpenChange(v)}>
			<DialogContent className="sm:max-w-md">
				...
				<Alert variant="destructive">
					...
					<AlertDescription>
						{t("hardDelete.warningDescription")}
					</AlertDescription>
				</Alert>
...
const HardDeleteContext = React.createContext<HardDeleteContextValue | null>(
	null,
);
...
export function HardDeleteFormProvider({
	children,
}: {
	...
	const openDialog = React.useCallback(
		(target: HardDeleteTarget, options?: OpenOptions) => {
...
export function useHardDeleteForm(): HardDeleteContextValue {
	...
	if (!ctx)
		throw new Error(
			"useHardDeleteForm must be used inside <HardDeleteFormProvider>",
		);
	return ctx;
}
