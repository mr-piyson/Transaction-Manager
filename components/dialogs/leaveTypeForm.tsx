...
export function LeaveTypeFormDialog({
	...
	const updateMutation = trpc.hr.leave.types.update.useMutation({
		onSuccess: () => {
			utils.hr.leave.types.list.invalidate();
			if (leaveType?.id)
				utils.hr.leave.types.byId.invalidate({ id: leaveType.id });
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						...
						<Field orientation="vertical">
							...
							<Select
								...
								onValueChange={(v) =>
									setValue("isPaid", v === "true", { shouldValidate: true })
								}
							>
								<SelectTrigger>
									<SelectValue />
...
const LeaveTypeFormContext =
	React.createContext<LeaveTypeFormContextValue | null>(null);
...
export function LeaveTypeFormProvider({
	children,
}: {
...
export function useLeaveTypeForm(): LeaveTypeFormContextValue {
	...
	if (!ctx)
		throw new Error(
			"useLeaveTypeForm must be used inside <LeaveTypeFormProvider>",
		);
	return ctx;
}
