...
export function AppSidebar({ ...props }: AppSidebarProps) {
  ...
  const items = useMemo(
    () => buildSidebarItems(t as (key: string) => string),
    [t],
  );
  ...
  const visibleItems = useMemo(() => {
    return tree.getItems().filter((item) => {
      ...
      if (searchQuery && matchingIds && !matchingIds.has(item.getId()))
        return false;
      return true;
    });
        ...
        <div className="overflow-auto">
          <Tree tree={tree} indent={indent} toggleIconType="chevron">
            {tree.getItems().map((item, _itemIndex) => {
              ...
              const active =
                !isFolder && itemData.href ? isActive(itemData.href) : false;
              const showSpinner =
                !isFolder && !!itemData.href && loading === itemData.href;

              if (searchQuery && matchingIds && !matchingIds.has(item.getId()))
                return null;

              const focusedItemIndex = visibleItems.findIndex(
                (vi) => vi.getId() === item.getId(),
              );
              const isFocused =
                focusedItemIndex === focusedIndex && focusedIndex >= 0;
            ...
            {searchQuery &&
              tree.getItems().filter((item) => matchingIds?.has(item.getId()))
                .length === 0 && (
