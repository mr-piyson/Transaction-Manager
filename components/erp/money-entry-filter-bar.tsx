...
export function MoneyEntryFilterBar({ kind, onCreate }: MoneyEntryFilterBarProps) {
  ...
  const [viewMode, setViewMode] = useQueryState('view', parseAsString.withDefault('table'));

  const { data: categories } = trpc.expenses.categories.list.useQuery(undefined, {
    enabled: isExpense,
  });
  const { data: customers } = trpc.customers.list.useQuery({ page: 1, limit: 500 }, { enabled: !isExpense });
