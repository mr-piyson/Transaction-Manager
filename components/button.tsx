...
export interface ButtonProps extends Omit<React.ComponentProps<'button'>, 'children'>, VariantProps<typeof buttonVariants> {
  asChild?: boolean;
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({ className, variant, size, asChild = false, children, mutation, isLoading: manualIsLoading, loadingText, isError: manualIsError, errorText = 'An error occurred', confirm, onClick, disabled, ...props }, ref) => {
  ...
  return (
    <Comp ref={ref} className={cn(buttonVariants({ variant, size, className }))} disabled={disabled || loading} onClick={handleClick} aria-busy={loading} {...props}>
      {loading ? (
        <>
