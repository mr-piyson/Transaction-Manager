...
export const DateInput = React.forwardRef<HTMLInputElement, DateInputProps>(
	function DateInput(
		...
		const tokens = React.useMemo(
			() => getDateFormatSegments(inputFormat, mode),
			[inputFormat, mode],
		);
		const totalDigits = React.useMemo(
			() => getSegmentTotalDigits(tokens),
			[tokens],
		);
		...
		const commitDate = React.useCallback(
			(date: Date) => {
				onChangeRef.current?.(
					mode === "datetime" ? date.toISOString() : format(date, "yyyy-MM-dd"),
				);
			},
			[mode],
		);
		...
		const otpElements: React.ReactNode[] = tokens.map((token, i) => {
			...
			const afterDivider =
				prevToken?.type === "separator" && prevToken.char !== " ";
			const el = (
				<InputOTPSlot
					key={`slot-${slotIndex}`}
					index={slotIndex}
			...
			<div className={cn("relative flex flex-col gap-1.5", className)}>
				...
				{(error || textError) && (
					<p className="text-xs text-destructive">
						{error ??
							`Enter a valid ${mode === "datetime" ? "date and time" : "date"}.`}
					</p>
				)}
			</div>
		);
...
export function DatePickerField({
	...
	return (
		<DatePicker value={value} onChange={handleChange} name={name} {...props} />
	);
}
