...
interface ImageUploadStepProps {
	items: ParsedItem[];
	onImagesChange: (
		images: ImportImage[] | ((prev: ImportImage[]) => ImportImage[]),
	) => void;
	images: ImportImage[];
}
...
export function ImageUploadStep({
	...
	const pendingCount = images.filter(
		(i) => i.uploadState === "pending" || i.uploadState === "uploading",
	).length;

	const skuSet = new Set(items.map((i) => i.sku));

	const uploadImage = useCallback(
		async (file: File): Promise<ImportImage> => {
		...
		<div className="space-y-4">
			<div className="text-center">
				...
				<p className="text-sm text-muted-foreground mt-1">
					Images are matched to items by filename (must match SKU, e.g.
					&quot;RAW-AL-001.jpg&quot;)
				</p>
			</div>
				...
				<div className="flex items-center gap-3 text-sm flex-wrap">
					...
					{pendingCount > 0 && (
						<span className="flex items-center gap-1 text-muted-foreground">
							<Loader2 className="size-3.5 animate-spin" /> {pendingCount}{" "}
							uploading
						</span>
					)}
					{unmatchedCount > 0 && (
			...
			{items.length > 0 && (
				<p className="text-xs text-muted-foreground">
					...
					{items.length - matchedCount > 0 &&
						` ${items.length - matchedCount} item(s) without images.`}
				</p>
			)}
		</div>
	);
