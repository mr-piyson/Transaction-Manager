...
export function ItemImportWizard() {
	...
	useEffect(() => {
		...
		for (const img of images) {
			const wasDone = prev.some(
				(p) => p.sku === img.sku && p.uploadState === "done",
			);
	...
	const handleImagesChange = useCallback(
		(updater: ImportImage[] | ((prev: ImportImage[]) => ImportImage[])) => {
			setImages(updater);
		},
		[],
	);
	...
	const handleBackFromImages = useCallback(() => {
		...
		setStep("upload-file");
		setItems((prev) =>
			prev.map((item) => ({ ...item, image: undefined, hasImage: false })),
		);
	}, []);
