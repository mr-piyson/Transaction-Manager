...
const TEMPLATE_ROW = [
	"RAW-AL-001",
	"Aluminum Sheet 2mm",
	"2mm thick aluminum sheet, 4x8 ft",
	"45.000",
	"28.500",
	"sheet",
	"Raw Materials",
...
function downloadCsvTemplate() {
	const csvContent = [TEMPLATE_COLUMNS.join(","), TEMPLATE_ROW.join(",")].join(
		"\n",
	);
		...
		async (file: File) => {
			...
			try {
				...
				for (let i = 0; i < jsonData.length; i++) {
					...
					const sku = String(
						row[
							Object.keys(columnMap).find((k) => columnMap[k] === "sku") || ""
						] || "",
					).trim();
				...
				if (items.length === 0) {
					setError(
						'No valid items found. Check that your file has a "SKU" column.',
					);
					return;
				}
