...
import { ShoppingCart, Truck, Calendar, DollarSign, Package, CheckCircle2, Clock } from 'lucide-react';
      ...
      <ListView
        ...
        cardRenderer={(po) => (
          <div className="flex items-center gap-4 px-4 py-4 hover:bg-accent/50 transition-colors border-b border-border/50 group">
            <div className={`p-3 rounded-2xl shrink-0 ${po.isReceived ? 'bg-green-500/10 text-green-600' : 'bg-amber-500/10 text-amber-600'}`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <Badge variant={po.isReceived ? 'default' : 'secondary'} className="text-[10px] h-4 uppercase tracking-tighter">
            ...
            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              {!po.isReceived ? (
                ...
                >
                  {receiveMutation.isPending ? <Clock className="size-3 animate-spin mr-1.5" /> : <CheckCircle2 className="size-3 mr-1.5" />}
                  Receive Stock
                </Button>
              ) : (
