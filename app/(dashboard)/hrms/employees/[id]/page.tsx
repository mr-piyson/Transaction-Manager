...
export default function EmployeeDetailPage() {
	...
	const {
		...
		isError,
		refetch,
	} = trpc.hr.employees.byId.useQuery(
		{ id: params.id },
		{ enabled: !!params.id },
	);
	...
	const updateStatusMutation = trpc.hr.employees.updateStatus.useMutation({
		onSuccess: () => {
			...
			utils.hr.employees.list.invalidate();
			utils.hr.employees.statusHistory.list.invalidate({
				employeeId: params.id,
			});
					...
					<div className="flex items-center gap-2">
						<DropdownMenu>
							...
							<DropdownMenuContent align="end">
								{STATUS_OPTIONS.filter((s) => s.value !== employee.status).map(
									(opt) => (
										<DropdownMenuItem
											key={opt.value}
											onClick={() => {
						...
						<Button
							...
							size="sm"
							onClick={() =>
								openEdit({ id: employee.id }, { onSuccess: () => refetch() })
							}
						>
						...
						<Button
							...
							onClick={() =>
								alert.delete({
									...
									description:
										"This will terminate and soft-delete this employee. This action cannot be undone.",
									confirmText: "Delete",
									onConfirm: async () => {
			...
			<div className="flex-1 p-6 space-y-6">
				...
				<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
					<Card>
						...
						<CardContent>
							<p className="text-2xl font-bold">
								{employee._count?.leaveRequests ?? 0}
							</p>
						</CardContent>
					</Card>
					<Card>
					...
					<Card>
						...
						<CardContent>
							<p className="text-2xl font-bold">
								{employee._count?.payrollItems ?? 0}
							</p>
						</CardContent>
					</Card>
					<Card>
					...
					<div className="lg:col-span-2 space-y-6">
						...
						{employee.directReports && employee.directReports.length > 0 && (
							<Card>
								<CardHeader>
									<CardTitle className="text-base flex items-center gap-2">
										<UserCircle className="size-4" /> Direct Reports (
			...
			<Dialog open={statusDialogOpen} onOpenChange={setStatusDialogOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Change status from{" "}
							<Badge variant="outline">{employee.status}</Badge> to{" "}
							<Badge
								variant="outline"
								className={
