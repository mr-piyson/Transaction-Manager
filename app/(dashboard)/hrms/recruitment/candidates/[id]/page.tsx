...
export default function CandidateDetailPage() {
	...
	const { data: candidate, isLoading: candidateLoading } =
		trpc.hr.recruitment.candidates.byId.useQuery({ id });
	const { data: interviewsData, refetch: refetchInterviews } =
		trpc.hr.recruitment.candidates.interviewRounds.list.useQuery({
			candidateId: id,
		});
	const { data: offerData, refetch: refetchOffer } =
		trpc.hr.recruitment.candidates.offer.get.useQuery({ candidateId: id });
	const { data: employeesData } = trpc.hr.employees.list.useQuery({});

	const createInterviewMutation =
		trpc.hr.recruitment.candidates.interviewRounds.create.useMutation({
			onSuccess: () => {
				refetchInterviews();
	...
	const updateInterviewMutation =
		trpc.hr.recruitment.candidates.interviewRounds.update.useMutation({
			onSuccess: () => {
				refetchInterviews();
				setUpdateInterviewDialogOpen(false);
	...
	const createOfferMutation =
		trpc.hr.recruitment.candidates.offer.create.useMutation({
			onSuccess: () => {
				refetchOffer();
	...
	const respondOfferMutation =
		trpc.hr.recruitment.candidates.offer.respond.useMutation({
			onSuccess: () => {
				refetchOffer();
				setRespondDialogOpen(false);
				toast.success(`Offer ${respondAction?.toLowerCase()}ed`);
			},
			onError: (e) => toast.error(e.message),
		});

	const withdrawOfferMutation =
		trpc.hr.recruitment.candidates.offer.withdraw.useMutation({
			onSuccess: () => {
				refetchOffer();
		...
		<div className="space-y-6 max-w-4xl">
			<Card>
				...
				<CardContent className="space-y-3 text-sm">
					<div className="grid grid-cols-2 gap-4">
						...
						<div>
							<strong>Job Posting:</strong> {c.jobPosting?.title ?? "—"}
						</div>
						<div>
							...
							<TableBody>
								{interviews.map((iv: any) => (
									<TableRow key={iv.id}>
										...
										<TableCell className="text-sm">
											{iv.interviewer?.user?.name ?? "—"}
										</TableCell>
										<TableCell className="text-sm">
											{iv.durationMin ? `${iv.durationMin}m` : "—"}
										</TableCell>
										<TableCell className="text-sm">
											{iv.result ?? "—"}
										</TableCell>
						...
						<div className="space-y-3">
							<div className="grid grid-cols-2 gap-4 text-sm">
								...
								<div>
									...
									{offer.salaryAmount
										? `${offer.currency ?? ""} ${offer.salaryAmount}`
										: "—"}
								</div>
								<div>
									...
									<Button
										...
										onClick={() => {
											if (confirm("Withdraw this offer?"))
												withdrawOfferMutation.mutate({ id });
										}}
									>
										Withdraw
									</Button>
			...
			>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Schedule a new interview round for this candidate.
						</DialogDescription>
					</DialogHeader>
					...
					<DialogFooter>
						...
						<Button
							onClick={() => {
								if (!interviewStage || !interviewScheduledAt) {
									toast.error("Stage and Scheduled At are required");
									return;
								}
								createInterviewMutation.mutate({
									candidateId: id,
									stage: interviewStage as
										| "PHONE_SCREEN"
										| "TECHNICAL"
										| "MANAGERIAL"
										| "HR"
			...
			>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Record the result, feedback, and score.
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4">
						<Field orientation="vertical">
					...
					<DialogFooter>
						<Button
							variant="outline"
							onClick={() => setUpdateInterviewDialogOpen(false)}
						>
							Cancel
						</Button>
						<Button
							onClick={() => {
								updateInterviewMutation.mutate({
									...
									result: (interviewResult || undefined) as
										| "CANCELLED"
										| "PENDING"
										| "PASSED"
										| "FAILED"
			...
			>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Create a job offer for this candidate.
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4">
						<Field orientation="vertical">
						...
						<div className="grid grid-cols-2 gap-4">
							<Field orientation="vertical">
								...
								<DatePicker
									...
									onChange={(v) => setOfferStartDate(v)}
								/>
							</Field>
							<Field orientation="vertical">
								...
								<DatePicker
									...
									onChange={(v) => setOfferExpiryDate(v)}
								/>
							</Field>
						</div>
						<Field orientation="vertical">
					...
					<DialogFooter>
						...
						<Button
							onClick={() => {
								if (!offerPosition || !offerSalary || !offerStartDate) {
									toast.error("Position, Salary, and Start Date are required");
									return;
								}
			...
			<Dialog open={respondDialogOpen} onOpenChange={setRespondDialogOpen}>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>
							{respondAction === "ACCEPT" ? "Accept Offer" : "Reject Offer"}
						</DialogTitle>
						<DialogDescription>
						...
						<Button
							onClick={() =>
								respondOfferMutation.mutate({
									id,
									status: respondAction === "ACCEPT" ? "ACCEPTED" : "REJECTED",
								})
							}
