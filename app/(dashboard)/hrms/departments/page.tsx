...
import { useDepartmentForm } from '@/components/dialogs';
import { trpc } from '@/lib/trpc/client';

export default function DepartmentsPage() {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState('');
  const { openCreate, openEdit } = useDepartmentForm();
  const limit = 25;

  const { data, isLoading, refetch } = trpc.hr.departments.list.useQuery({
    page,
    limit,
    search: search || undefined,
  });

  const deleteMutation = trpc.hr.departments.delete.useMutation({
    onSuccess: () => {
      refetch();
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="flex items-center justify-center py-20">
          <Empty>
            <EmptyHeader>
              <EmptyMedia><Building2 className="size-16 text-muted-foreground" /></EmptyMedia>
              <EmptyTitle>No departments</EmptyTitle>
              <EmptyDescription>No departments found for the selected filters.</EmptyDescription>
            </EmptyHeader>
          </Empty>
        </div>
      ) : (
                ...
                <TableBody>
                  {records.map((record: any) => (
                    <TableRow key={record.id}>
                      ...
                      <TableCell className="text-sm">{record.manager?.user?.name || '—'}</TableCell>
                      <TableCell className="text-sm">{record.parent?.name || '—'}</TableCell>
                      <TableCell className="text-sm">
                        <Badge variant="secondary">
                          {record._count?.children ?? 0} sub / {record._count?.employees ?? 0} emp
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <DropdownMenu>
