...
function parseDeviceInfo(
	...
	const browser = /chrome/i.test(lower)
		? t("sessions.browserChrome")
		: /firefox/i.test(lower)
			? t("sessions.browserFirefox")
			: /safari/i.test(lower)
				? t("sessions.browserSafari")
				: /edge/i.test(lower)
					? t("sessions.browserEdge")
					: "";
...
export default function SessionsSettingsPage() {
	...
	const isLoadingMutation = revokeSession.isPending || revokeOther.isPending;
							...
							<div className="rounded-lg border-2 border-primary/40 p-4">
								<div className="flex items-start justify-between gap-4">
									<div className="flex items-start gap-3 min-w-0">
										{(() => {
											const info = parseDeviceInfo(currentSession.userAgent, t);
											...
											<p className="text-xs text-muted-foreground">
												...
												{currentSession.expiresAt &&
													t("sessions.expiresLabel", {
														date: formatTime(currentSession.expiresAt),
													})}
											</p>
										</div>
									</div>
									...
									<div className="flex items-start gap-3 min-w-0">
										...
										<div className="min-w-0">
											...
											<p className="text-xs text-muted-foreground">
												...
												{session.expiresAt &&
													t("sessions.expiresLabel", {
														date: formatTime(session.expiresAt),
													})}
											</p>
										</div>
									</div>
