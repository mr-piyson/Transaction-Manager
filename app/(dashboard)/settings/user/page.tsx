...
export default function UserSettingsPage() {
	...
	const groupedPermissions = useMemo(() => {
		...
		return (me.permissionsList as any[]).reduce<Record<string, any[]>>(
			(acc, p: any) => {
	...
	const handleChangePassword = useCallback(() => {
		...
		if (!passwordForm.newPassword || passwordForm.newPassword.length < 8) {
			toast.error(
				t("errors.minLength", { field: t("users.newPassword"), min: 8 }),
			);
			return;
		}
	...
	return (
		<div className="h-full space-y-6">
			<SectionCard title={t("auth.name")} description={t("common.details")}>
						...
						<div className="flex items-start gap-4 p-4 rounded-xl border bg-card text-card-foreground shadow-sm">
							...
							>
								{(() => {
									...
									return me?.roleDetails?.icon
										? (iconMap[me.roleDetails.icon] ?? "⚙")
										: "👤";
								})()}
							</div>
