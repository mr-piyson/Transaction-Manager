...
function fromSettings(
	...
	return {
		probationPeriodMonths: Number(
			s[toKey("probationPeriodMonths")] ?? DEFAULTS.probationPeriodMonths,
		),
		workingDaysPerWeek: Number(
			s[toKey("workingDaysPerWeek")] ?? DEFAULTS.workingDaysPerWeek,
		),
		overtimeRateMultiplier: Number(
			s[toKey("overtimeRateMultiplier")] ?? DEFAULTS.overtimeRateMultiplier,
		),
		leaveCarryForwardMaxDays: Number(
			s[toKey("leaveCarryForwardMaxDays")] ?? DEFAULTS.leaveCarryForwardMaxDays,
		),
		attendanceGracePeriodMinutes: Number(
			s[toKey("attendanceGracePeriodMinutes")] ??
				DEFAULTS.attendanceGracePeriodMinutes,
		),
...
export default function HrmsSettingsPage() {
	...
	const hasChanges = useMemo(
		() =>
			...
			form.attendanceGracePeriodMinutes !==
				settings.attendanceGracePeriodMinutes ||
	...
	const handleSave = useCallback(() => {
		const entries: { key: string; value: string }[] = [
			{
				key: toKey("probationPeriodMonths"),
				value: String(form.probationPeriodMonths),
			},
			{
				key: toKey("workingDaysPerWeek"),
				value: String(form.workingDaysPerWeek),
			},
			{
				key: toKey("overtimeRateMultiplier"),
				value: String(form.overtimeRateMultiplier),
			},
			{
				key: toKey("leaveCarryForwardMaxDays"),
				value: String(form.leaveCarryForwardMaxDays),
			},
			{
				key: toKey("attendanceGracePeriodMinutes"),
				value: String(form.attendanceGracePeriodMinutes),
			},
				...
				<Field label={t("settings.hrms.carryForwardMax")}>
					<Input
						...
						onChange={(e) =>
							setForm((p) => ({
								...p,
								leaveCarryForwardMaxDays: Number(e.target.value),
							}))
						}
					/>
				</Field>
				<Field label={t("settings.hrms.autoApprove")}>
					<div className="flex items-center gap-2">
						<Switch
							...
							onCheckedChange={(v) =>
								setForm((p) => ({ ...p, autoApproveLeave: v }))
							}
						/>
						<span className="text-sm text-muted-foreground">
							{form.autoApproveLeave
								? t("common.enabled")
								: t("common.disabled")}
						</span>
					</div>
				...
				<Field label={t("settings.hrms.gracePeriod")}>
					<Input
						...
						onChange={(e) =>
							setForm((p) => ({
								...p,
								attendanceGracePeriodMinutes: Number(e.target.value),
							}))
						}
					/>
				</Field>
