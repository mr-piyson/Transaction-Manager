...
export default function InvoicesLayout({ children }: { children?: React.ReactNode }) {
  ...
  const activeItem = segments[3];
  const isListRoute = segments.length === 3 || (segments.length === 3 && !segments[2]);
  ...
  const deleteMutation = trpc.invoices.delete.useMutation({
    onSuccess: () => {
      ...
      if (activeItem) router.push('/erp/invoices');
    },
          ...
          return (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => router.push(`/erp/invoices/${item.id}`)}>
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden">
      <Header title={t('layout.invoices')} icon={<Receipt className="size-5" />} />
                ...
                <Tabs value={statusFilter} onValueChange={setStatusFilter}>
                  <TabsList className="h-auto justify-start">
                    {STATUS_FILTERS.map((f) => (
                      <TabsTrigger key={f.value} value={f.value} className="text-xs px-3 py-1">
                        {f.label}
                      </TabsTrigger>
                    ))}
                  </TabsList>
