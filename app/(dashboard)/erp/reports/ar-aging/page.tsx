...
export default function ARAgingPage() {
  ...
  const invoiceList = useMemo(() => {
    if (selectedBucketData) return selectedBucketData.invoices;
    if (!data) return [];
  ...
  const columnDefs = useMemo<ColDef[]>(
    () => [
      ...
      { field: 'customerName', headerName: t('common.customer'), flex: 1, minWidth: 150 },
      {
        field: 'total',
  ...
  return (
    <ReportLayout title={t('reports.arAging')} icon={<Receipt className="size-5" />}>
        ...
        {isLoading
          ...
          : data?.buckets.map((bucket, i) => (
              <ReportKpiCard
                ...
                count={bucket.count}
                countLabel={t('reports.invoiceCount', { count: bucket.count })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 print:shadow-none">
          <CardHeader>
            ...
            <CardDescription>{t('reports.arAgingDescription')}</CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-12">
                {t('reports.noOutstandingReceivables')}
              </p>
            ) : (
              ...
              <div className="space-y-3">
                ...
                <div className="pt-3 border-t">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium">{t('reports.totalInvoices')}</span>
      ...
      <Card className="print:shadow-none">
        <CardHeader className="pb-3 flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              {invoiceList.length} {t('reports.outstandingInvoices')}
            </CardDescription>
          </div>
          <div className="flex items-center gap-2 print:hidden">
            ...
            <ReportCsvExportButton gridApiRef={gridApiRef} filename="ar-aging" />
          </div>
        </CardHeader>
