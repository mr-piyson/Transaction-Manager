...
export default function APagingPage() {
  ...
  const columnDefs = useMemo<ColDef[]>(
    () => [
      ...
      {
        field: 'supplierName',
        headerName: t('common.supplier') ?? 'Supplier',
        flex: 1,
        minWidth: 150,
      },
      {
      ...
      {
        field: 'amountOwed',
        headerName: t('invoices.outstanding') ?? 'Amount Owed',
        width: 130,
        type: 'rightAligned',
        valueFormatter: (p) => format(p.value),
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-background to-muted/20 pb-12">
      <Header
        title={t('reports.apAging') ?? 'Accounts Payable Aging'}
          ...
          <Card className="lg:col-span-2">
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                ...
                {t('reports.apAging') ?? 'AP Aging Distribution'}
              </CardTitle>
              <CardDescription>
                {t('reports.apAgingDescription') ?? 'Outstanding payables by aging bucket'}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                ...
                <p className="text-sm text-muted-foreground text-center py-12">
                  {t('reports.noOutstandingPayables') ?? 'No outstanding payables'}
                </p>
              ) : (
                ...
                <div className="space-y-3">
                  {data?.buckets.map((bucket) => (
                    ...
                    >
                      <div className="text-left">
                        ...
                        <div className="text-xs text-muted-foreground">{bucket.count} POs</div>
                      </div>
                  ...
                  <div className="pt-3 border-t">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-bold">{data?.totalCount} POs</span>
                    </div>
          ...
          <CardHeader className="pb-3 flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-base flex items-center gap-2">
                ...
                {selectedBucket ? `${selectedBucket} - ` : ''}{t('reports.purchaseOrders') ?? 'Purchase Orders'}
              </CardTitle>
              <CardDescription>
                {poList.length} {t('reports.outstandingPOs') ?? 'outstanding purchase orders'}
              </CardDescription>
            </div>
            <div className="flex items-center gap-2">
              ...
              {gridApiRef.current && (
                <Button
                  ...
                  onClick={() => gridApiRef.current?.exportDataAsCsv({ fileName: 'ap-aging' })}
                >
