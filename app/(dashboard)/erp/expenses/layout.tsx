...
import { Edit, Eye, MoreHorizontal, ShieldAlert, Trash2, Wallet } from 'lucide-react';
...
export default function ExpensesLayout({ children }: { children?: React.ReactNode }) {
  ...
  const [categoryFilter] = useQueryState('categoryId', parseAsString.withDefault(''));
  ...
  const listColumnDefs = useMemo<ColDef[]>(
        ...
        cellRenderer: (params: { data: any }) => {
          ...
          return (
            <ContextMenu>
              ...
              <ContextMenuContent>
                <ContextMenuItem onClick={() => router.push(`/erp/expenses/${item.id}`)}>
                ...
                {isSuperAdmin && (
                  <>
                    ...
                    <ContextMenuItem
                      onClick={() =>
                        openHardDelete({ kind: 'expense', id: item.id, title: item.description })
                      }
                      variant="destructive"
                    >
    ...
    [activeItem, router, t, handleEdit, handleDelete, isSuperAdmin, openHardDelete],
  );
  ...
  const tableColumnDefs = useMemo<ColDef[]>(
        ...
        cellRenderer: (params: { data: any }) => {
          ...
          return (
            <div className="flex flex-col justify-center overflow-hidden">
              ...
              {item.reference && (
                <span className="truncate text-[11px] text-muted-foreground">{item.reference}</span>
              )}
            </div>
          );
        },
          ...
          return (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => router.push(`/erp/expenses/${item.id}`)}>
                  <Eye className="size-4 mr-2" />
                  {t('common.viewDetails')}
                </DropdownMenuItem>
    ...
    [t, formatDate, format, handleEdit, handleDelete, isSuperAdmin, openHardDelete, router],
  );
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden">
      <Header title={t('layout.expenses')} icon={<Wallet className="size-5" />} />
