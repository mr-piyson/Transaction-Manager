      ...
      <div className="flex items-center justify-center h-[60vh]">
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>{error?.message ?? t('expenses.doesNotExist')}</EmptyDescription>
          </EmptyHeader>
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => router.push('/erp/expenses')}>
  ...
  return (
    <div className="flex flex-col h-screen">
      <header className="flex h-14 items-center gap-2 px-2 border-b bg-background/95 backdrop-blur-md sticky top-0 z-50 shrink-0">
        <Button variant="ghost" size="icon" onClick={() => router.push('/erp/expenses')}>
          <ArrowLeft className="size-5" />
        </Button>
        <span className="text-muted-foreground">|</span>
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <Wallet className="size-5 text-muted-foreground shrink-0" />
          <h1 className="text-xl font-semibold truncate">{expense.description}</h1>
        ...
        {po && (
          <Card>
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground">{t('expenses.linkedPurchaseOrder')}</p>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center gap-2 flex-wrap">
                <Badge
                  variant="outline"
                  className={JE_STATUS_COLORS[expense.journalEntry.status] ?? ''}
                >
                  {expense.journalEntry.status}
                </Badge>
                <span className="text-sm font-medium">{expense.journalEntry.entryNumber}</span>
                <span className="text-sm text-muted-foreground">
                  {t('expenses.postedOn', { date: formatDate(expense.journalEntry.date) })}
                </span>
