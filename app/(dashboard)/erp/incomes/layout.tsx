...
import { Banknote, Edit, Eye, MoreHorizontal, ShieldAlert, Trash2 } from 'lucide-react';
...
export default function IncomesLayout({ children }: { children?: React.ReactNode }) {
  ...
  const [customerFilter] = useQueryState('customerId', parseAsString.withDefault(''));
        ...
        cellRenderer: (params: { data: any }) => {
          ...
          return (
            <ContextMenu>
              ...
              <ContextMenuContent>
                <ContextMenuItem onClick={() => router.push(`/erp/incomes/${item.id}`)}>
                ...
                {isSuperAdmin && (
                  <>
                    ...
                    <ContextMenuItem
                      onClick={() =>
                        openHardDelete({ kind: 'income', id: item.id, title: item.description })
                      }
                      variant="destructive"
                    >
          ...
          return (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => router.push(`/erp/incomes/${item.id}`)}>
                  <Eye className="size-4 mr-2" />
                  {t('common.viewDetails')}
                </DropdownMenuItem>
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden">
      <Header title={t('layout.incomes')} icon={<Banknote className="size-5" />} />
