      ...
      <div className="flex items-center justify-center h-[60vh]">
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>{error?.message ?? t('incomes.doesNotExist')}</EmptyDescription>
          </EmptyHeader>
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => router.push('/erp/incomes')}>
  ...
  return (
    <div className="flex flex-col h-screen">
      <header className="flex h-14 items-center gap-2 px-2 border-b bg-background/95 backdrop-blur-md sticky top-0 z-50 shrink-0">
        <Button variant="ghost" size="icon" onClick={() => router.push('/erp/incomes')}>
          <ArrowLeft className="size-5" />
        </Button>
        <span className="text-muted-foreground">|</span>
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <Banknote className="size-5 text-muted-foreground shrink-0" />
          <h1 className="text-xl font-semibold truncate">{income.description}</h1>
          {income.customer && <Badge variant="secondary">{income.customer.name}</Badge>}
        </div>
        <div className="flex items-center gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              {isSuperAdmin && (
                <DropdownMenuItem variant="destructive" onClick={handleHardDelete}>
        ...
        {inv && (
          <Card>
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground">{t('incomes.linkedInvoice')}</p>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-sm font-medium">{income.journalEntry.entryNumber}</span>
                <span className="text-sm text-muted-foreground">
                  {t('incomes.postedOn', { date: formatDate(income.journalEntry.date) })}
                </span>
                {income.journalEntry.postedAt && (
                  <span className="text-sm text-muted-foreground">
                    {t('incomes.postedAt', { date: formatDateTime(income.journalEntry.postedAt) })}
                  </span>
                )}
              </div>
            </CardContent>
