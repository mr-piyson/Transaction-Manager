  ...
  return (
    <div className="flex items-start gap-4 mb-4 relative">
      <div className="flex flex-col items-center shrink-0">
        ...
        {!isLast && <div className="w-0.5 flex-1 min-h-12 bg-linear-to-b from-primary/40 to-primary/5 mt-2" />}
      </div>
...
export default function ErpDashboard() {
  ...
  const invoiceList = Array.isArray(recentInvoices) ? recentInvoices : ((recentInvoices as any)?.data ?? []);

  const pendingPOs = poList.filter((p: any) => ["DRAFT", "PENDING_APPROVAL", "APPROVED"].includes(p.status)).length;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-linear-to-b from-background to-muted/20 pb-12">
      <Header title={t("dashboard.title")} />

      <main className="flex-1 p-4 lg:p-8 space-y-8 max-w-360 mx-auto w-full">
        <div className="relative overflow-hidden rounded-2xl bg-linear-to-br from-primary via-primary/90 to-primary/70 p-6 lg:p-8 text-primary-foreground shadow-2xl shadow-primary/30">
        ...
        <section>
          ...
          <div className="space-y-0">
            <WorkflowStep
              ...
              stats={[
                {
                  label: t("dashboard.step1StatsPending"),
                  value: String(pendingPOs),
                },
                {
                  label: t("dashboard.step1StatsTotalPOs"),
                  value: String(summary?.purchases.count ?? 0),
                },
                {
            ...
            <WorkflowStep
              ...
              stats={[
                {
                  label: t("dashboard.step2StatsAwaiting"),
                  value: String(orderedPOs),
                },
                {
                  label: t("dashboard.step2StatsReceived"),
                  value: String(receivedPOs),
                },
                {
                  label: t("dashboard.step2StatsInStock"),
            ...
            <WorkflowStep
              ...
              stats={[
                {
                  label: t("dashboard.step3StatsInvoiced"),
                  value: String(summary?.revenue.count ?? 0),
                },
                {
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-3">
            <StatCard
              ...
              sub={t("dashboard.lowStockItems", {
                n: summary?.inventory.lowStockCount ?? 0,
              })}
              ...
              trend={
                summary?.inventory.lowStockCount
                  ? {
                      dir: "down",
                      label: t("dashboard.needsReordering", {
                        n: summary.inventory.lowStockCount,
                      }),
                    }
                  : undefined
              }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {po.supplier?.name ?? "—"} · {po.date ? formatShortDate(po.date) : "—"}
                        </p>
                      </div>
          ...
          <Card>
            <CardHeader className="pb-3 flex flex-row items-center justify-between">
              ...
              <Button variant="ghost" size="sm" className="text-xs gap-1" onClick={() => router.push("/erp/invoices")}>
                {t("dashboard.viewAll")} <ArrowRight className="size-3" />
              </Button>
            </CardHeader>
            <CardContent className="p-0">
              {invoiceList.length === 0 ? (
                <div className="flex flex-col items-center py-8 text-muted-foreground">
                  ...
                  <Button variant="link" size="sm" className="mt-1" onClick={() => router.push("/erp/invoices")}>
                  ...
                  {invoiceList.map((inv: any) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {inv.customer?.name ?? "—"} · {inv.date ? formatShortDate(inv.date) : "—"}
                        </p>
                      </div>
                      <span className="text-sm font-semibold shrink-0">
                  ...
                  {topItems.slice(0, 5).map((item: any, i: number) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {item.sku ?? "—"} · {currencyFormat(item.salesPrice)}
                          {t("dashboard.perUnit")}
                        </p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-sm font-semibold">
                          {item.totalSold.toFixed(0)} {t("dashboard.sold")}
                        </p>
                        <p className="text-xs text-muted-foreground">{currencyFormat(item.totalRevenue)}</p>
                      </div>
                    </div>
                  ))}
                </div>
