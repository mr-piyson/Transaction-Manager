    ...
    <div className="flex items-center gap-3 p-3 h-20 w-full bg-transparent group transition-colors">
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center gap-1.5">
        <div className="flex items-center gap-2">
          <p className="font-semibold truncate text-sm text-foreground uppercase tracking-tight leading-none">{data.title}</p>
          <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-secondary text-secondary-foreground border border-border">v{data.version}</span>
        </div>
      ...
      <div className="text-right flex flex-col justify-center items-end min-w-27.5 gap-1">
        ...
        <div className={cn('text-[10px] font-bold uppercase tracking-widest flex items-center gap-1.5 px-2 py-0.5 rounded-full border', data.active ? 'text-emerald-600 bg-emerald-50/50 border-emerald-200 dark:bg-emerald-950/20 dark:border-emerald-800' : 'text-muted-foreground bg-muted/50 border-transparent')}>
          <span className={cn('size-1.5 rounded-full animate-pulse', data.active ? 'bg-emerald-500' : 'bg-slate-400')} />
