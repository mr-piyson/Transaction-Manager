...
function classifyClipboardError(
	error: unknown,
	...
	if (context.secureContext === false) {
		return {
			...
			message:
				"Clipboard access requires a secure connection (HTTPS or localhost). This page is served over an insecure connection.",
		};
	}
...
export const ClipboardService = {
	copy: async (text: string): Promise<ClipboardResult> => {
		...
		if (diagnostics.clipboardApiSupported && diagnostics.secureContext) {
			try {
				...
				return classifyClipboardError(error, {
		...
		if (!diagnostics.secureContext) {
			return {
				...
				code: ClipboardErrorCode.INSECURE_CONTEXT,
				message:
					"Cannot copy to clipboard over an insecure connection (HTTP). Use HTTPS or access via localhost.",
			};
		}
	...
	read: async (): Promise<ClipboardResult & { text?: string }> => {
		...
		if (!diagnostics.clipboardApiSupported || !diagnostics.secureContext) {
			...
			return {
				...
				code: ClipboardErrorCode.NOT_SUPPORTED,
				message:
					"Clipboard read requires a secure context and is not available on this connection.",
			};
		}
	...
	diagnose: (): ClipboardDiagnostics => {
		...
		if (secureContext) {
			recommendedFix =
				"Your connection is secure. Clipboard should work normally.";
		} else if (localhost) {
			recommendedFix =
				"localhost is treated as a secure context. Clipboard should work.";
		} else if (https) {
			recommendedFix =
				"HTTPS should be a secure context. Check for mixed content or iframe issues.";
		} else {
			recommendedFix =
				"Clipboard access requires a secure context. Use HTTPS instead of HTTP, or access via localhost. Browser security policies intentionally block clipboard access on insecure origins and this cannot be overridden by JavaScript.";
		}
